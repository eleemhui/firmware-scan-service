...
MongoDB was chosen for its flexible document model, which accommodates variable-size firmware metadata without schema changes. It supports atomic inserts and updates that enable distributed deduplication without application-level locking. When scaling is needed, MongoDB offers built-in replica sets and native sharding as a straightforward upgrade path.
...
The Watchdog is a background thread (goroutine) that runs in the API process (for simplicity). The Watchdog will cover certain scenarios like orphaned/stale scans that can occur if a worker dies or the message queue fails and messages are lost. After a timeout period has passed, stale/orphaned scan jobs will be requeued. 

### 5. CVE Records contain a reference to the first and last scan to report it and a total detected count
When a vulnerability is detected in a scan an atomic `upsert` operation is performed to either create a new CVE record with a count of 1 or update the existing record's count with a thread-safe `inc` operation.
...
`RegisterScan` calls `InsertOne` with a unique compound index on `(device_id, binary_hash)`. If a new scan record is created it will publish one message to RabbitMQ. If MongoDB returns a duplicate key error, the existing document is fetched and returned to the caller with nothing added to the queue.  

- New scan → `201 Created` + message published
- Duplicate → `200 OK` + existing record returned, no message published

The worker additionally guards against duplicate processing by checking the `status = "scheduled"` like this:
...
```
  ...
  Failure paths:
    ...
    Worker returns error        → Nack the message with requeue=false → Dead Letter Queue
    Scan stuck in "started"     → Watchdog resets to "scheduled" after staleThreshold and republishes
    Message lost from queue     → Watchdog status already "scheduled" → re-publishes after orphanedThreshold
```
...
```
_id              ObjectID  generated by MongoDB client at insert time
device_id        string
...
first_detected_by string    hex ObjectID of the scan that first detected this CVE (empty if submitted via API)
last_detected_by  string    hex ObjectID of the scan that most recently detected this CVE (empty if submitted via API)
...
| Component | Behaviour |
|-----------|-----------|
| **API** | Stateless — scale horizontally with `--scale api=N` (Requires a load-balancer). Each replica independently inserts and publishes. MongoDB's unique index serialises duplicate requests without API coordination. |
...
- **Increase worker replicas** — raise `deploy.replicas` in `docker-compose.yml` or use a Kubernetes `Deployment` replica count.
...
- **MongoDB sharding** — shard the `firmware_scans` collection on `device_id`. Queries and writes for the same device route to the same shard, keeping the unique index local to one shard. Shard the vulnerabilities collection on `_id` if writes become too heavy for one node.
...
- **API Gateway + rate limiting** — fronting the API with a gateway can add rate limiting, authentication, and TLS termination before requests reach the Go service.
